...
def remove_block(terraform_dir: str, file: str, start_line: int, end_line: int) -> BlockEdit:
    """
...
def set_block_attribute(
    terraform_dir: str, file: str, start_line: int, end_line: int, attribute: str, new_value: str
) -> BlockEdit:
    """
...
def _check_range(lines: List[str], start_line: int, end_line: int, path: Path) -> None:
    if not (1 <= start_line <= end_line <= len(lines)):
        raise TerraformEditError(
            f"Invalid line range {start_line}-{end_line} for {path} " f"({len(lines)} lines)"
        )


def _diff(old_lines: List[str], new_lines: List[str], file: str) -> str:
    return "".join(
        difflib.unified_diff(old_lines, new_lines, fromfile=f"a/{file}", tofile=f"b/{file}")
    )
