...
def test_service_cost_sum_matches_total():
    ...
    assert validate_service_breakdown({"EC2": 14000, "RDS": 9000, "S3": 4000}, 27000) == 0.0
...
def test_claim_percentage_matches_dataset():
    ...
    with pytest.raises(FinOpsInvariantError):
        validate_claim_percentage(40, detected_waste_monthly=6000, spend_monthly=80000)
    # Claim aligné sur les données : passe
    assert validate_claim_percentage(7.5, detected_waste_monthly=6000, spend_monthly=80000) == 7.5
    # Exercice 1 : 15 % annoncé, 15 % recalculé
    assert validate_claim_percentage(15, detected_waste_monthly=7500, spend_monthly=50000) == 15
