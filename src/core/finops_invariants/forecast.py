...
def linear_forecast(current_spend: float, days_elapsed: float, days_in_month: float) -> float:
    ...
    if days_elapsed <= 0 or days_in_month <= 0:
        raise FinOpsInvariantError("days_elapsed and days_in_month must be positive")
    if days_elapsed > days_in_month:
        raise FinOpsInvariantError("days_elapsed cannot exceed days_in_month")
    return current_spend / days_elapsed * days_in_month


def forecast_after_remediation(
    forecast: float, validated_low_risk_savings: float, current_spend_mtd: float
) -> float:
