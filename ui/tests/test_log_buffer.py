...
class TestRingBufferHandler(unittest.TestCase):
    ...
    def test_search_matches_message_and_logger(self):
        ...
        self.assertEqual(
            len(self.handler.query(search='SYNC')['entries']), 1)
        self.assertEqual(
            len(self.handler.query(search='app.db')['entries']), 1)
        self.assertEqual(
            len(self.handler.query(search='nomatch')['entries']), 0)
    ...
    def test_exception_appended_to_message(self):
        try:
            ...
            record = make_record('failed', logging.ERROR,
...
class TestInstallCapture(unittest.TestCase):
    ...
    def test_no_duplicate_capture_through_propagation(self):
        ...
        for name in CAPTURED_LOGGERS + ['app.module.sub']:
            ...
            entries = [e for e in handler.query(after_id=before)['entries']
                       if 'dedupe-check' in e['message']]
            self.assertEqual(len(entries), 1,
                             f"logger {name!r} captured {len(entries)} times")
