...
# ---------------------------------------------------------------------------
# Pricing sanity avancé — métadonnées requises et types de saving
# ---------------------------------------------------------------------------
...
def validate_pricing_metadata_complete(
        item: Dict[str, Any],
        required_fields: tuple = REQUIRED_PRICING_METADATA_FIELDS) -> bool:
...
def validate_estimated_cost_matches_unit_price(
        unit_price_hourly: float, hours: float, estimated_cost: float,
...
def validate_resize_saving(current_monthly_cost: float,
                          recommended_monthly_cost: float,
                          potential_saving: float) -> float:
...
def validate_schedule_saving(full_monthly_cost: float,
    ...
    if not 0 <= reduced_runtime_ratio <= 1:
        raise FinOpsInvariantError(
            "reduced_runtime_ratio must be between 0 and 1")
    expected = full_monthly_cost * (1 - reduced_runtime_ratio)
    return validate_cost_within_tolerance(
        claimed_saving, expected, tolerance_pct, label='schedule_saving')


def validate_delete_saving(is_delete_safe: bool, saving: float,
                          full_monthly_cost: float) -> float:
...
# ---------------------------------------------------------------------------
# Remediation workflow safety
# ---------------------------------------------------------------------------

REMEDIATION_STATES = (
    'detected', 'validated', 'approval_required', 'approved', 'scheduled',
    'executed', 'rolled_back', 'ignored', 'expired',
)


def validate_approval_required(action: str, environment: Optional[str],
                               risk: str, approval_required: bool) -> bool:
...
def validate_auto_execution(execution_mode: str, owner: Optional[str],
                           rollback_plan: Optional[str]) -> bool:
    ...
    if execution_mode == 'auto':
        if not owner:
            raise FinOpsInvariantError(
                "Auto execution requires a known owner")
        if not rollback_plan:
            raise FinOpsInvariantError(
                "Auto execution requires a documented rollback plan")
    return True


def validate_completed_status_requires_audit_log(
        status: str, audit_log_id: Optional[str]) -> bool:
    if status == 'executed' and not audit_log_id:
        raise FinOpsInvariantError(
            "Status 'executed' requires an audit_log_id")
    return True


def validate_realized_savings_status(status: str,
                                     realized_monthly_saving: float) -> float:
...
def validate_no_saving_when_ignored_or_expired(status: str,
                                               saving: float) -> bool:
    if status in ('ignored', 'expired') and saving and saving > 0:
        raise FinOpsInvariantError(
            f"Status '{status}' recommendation cannot count {saving} as a "
            f"saving")
    return True


def validate_approval_identity(approval_required: bool,
                              approved_by: Optional[str]) -> bool:
    if approval_required and not approved_by:
        raise FinOpsInvariantError(
            "approval_required=True but no approved_by identity recorded")
    return True


def validate_execution_timestamp(status: str,
                                executed_at: Optional[str]) -> bool:
    if status == 'executed' and not executed_at:
        raise FinOpsInvariantError(
            "Status 'executed' requires an executed_at timestamp")
    return True


def validate_slack_approval_trace(approval_channel: Optional[str],
                                 trace_id: Optional[str]) -> bool:
    if approval_channel == 'slack' and not trace_id:
        raise FinOpsInvariantError(
            "Slack approval requires a trace_id to stay auditable")
    return True


def validate_terraform_pr_remediation(execution_mode: str,
                                      pull_request_url: Optional[str]) -> bool:
    if execution_mode == 'terraform_pr' and not pull_request_url:
        raise FinOpsInvariantError(
            "execution_mode 'terraform_pr' requires a pull_request_url")
    return True
...
FORBIDDEN_WORDS_FOR_POTENTIAL_CLAIMS = FORBIDDEN_WORDS_FOR_POTENTIAL_CLAIMS + (
    'risk-free', 'confirmed savings', 'permanent savings', 'save automatically',
)


def validate_dashboard_headline(headline: str, realized_savings_monthly: float,
                                executed_actions_count: int) -> str:
    ...
    if 'realized' in lowered and executed_actions_count == 0:
        raise FinOpsInvariantError(
            f"Headline claims 'realized' with zero executed actions: "
            f"{headline!r}")
    return headline


def validate_annualized_claim_assumptions(claim_type: str,
    ...
    if claim_type == 'annualized_potential' and not assumptions:
        raise FinOpsInvariantError(
            "Annualized potential claim must state its assumptions")
    return True


# ---------------------------------------------------------------------------
# Audit AWS réel — comparaison read-only avec les sources externes
# ---------------------------------------------------------------------------

def validate_within_tolerance_pct(actual: float, reference: float,
                                  tolerance_pct: float,
    ...
    if reference == 0:
        raise FinOpsInvariantError(
            f"{label} reference is zero: cannot compute a tolerance")
...
def validate_resources_exist_in_aws(recommended_resource_ids: List[str],
                                    aws_resource_ids: set) -> bool:
...
def validate_read_only_audit(api_calls: List[str],
                             forbidden_write_actions: set) -> bool:
    violations = set(api_calls) & forbidden_write_actions
    if violations:
        raise FinOpsInvariantError(
            f"Write API(s) called during a read-only audit: {violations}")
    return True


def validate_audit_trace(trace_id: Optional[str], started_at: Optional[str],
                         completed_at: Optional[str]) -> bool:
    missing = [name for name, value in (
        ('trace_id', trace_id), ('started_at', started_at),
        ('completed_at', completed_at)) if not value]
    if missing:
        raise FinOpsInvariantError(
            f"Audit run missing required trace fields: {missing}")
    return True
