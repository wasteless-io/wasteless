...
def validate_policy(data: Any) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if expected is None:
            ...
            errors.append(f"section {key!r} must be a {expected.__name__}, "
    ...
    for section, key in _LIMITED_KEYS:
        if isinstance(config.get(section), dict) and key in config[section]:
            try:
                config[section][key] = validate_config_value(
    ...
    if isinstance(config.get('auto_remediation'), dict):
        ...
        if not isinstance(actions, dict) or \
                not all(isinstance(v, bool) for v in actions.values()):
    ...
    if isinstance(config.get('schedule'), dict):
        ...
        if not isinstance(hours, list) or \
