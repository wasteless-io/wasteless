...
class EC2Remediator:
    ...
    def acquire_lock(self) -> bool:
        ...
        try:
            ...
            if acquired:
                ...
                logger.warning("⚠️  Could not acquire lock - another remediator is running")
    ...
    def create_rollback_snapshot(
        self, action_log_id: int, instance_id: str, instance_details: Dict
    ) -> int:
        """
    ...
    def stop_instance(
        ...
        try:
            ...
            if not safeguard_result["safe_to_proceed"]:
                raise SafeguardException(f"Safeguard check failed: {safeguard_result['reason']}")
            ...
            if self.dry_run:
                ...
                logger.info("   New state: stopping")
    ...
    def start_instance(self, instance_id: str, reason: str = "Manual rollback") -> Dict:
        """
        ...
        result = {"success": False, "instance_id": instance_id, "action": "start", "error": None}
        ...
        try:
            ...
            if instance_details["state"] != "stopped":
                raise Exception(
                    f"Instance is {instance_details['state']}, " f"cannot start (must be stopped)"
                )
    ...
    def process_pending_recommendations(self, limit: int = 10) -> List[Dict]:
        ...
        if not self.acquire_lock():
            logger.error("❌ Cannot process recommendations - another remediator is running")
            return []
