...
def _insert_waste(cur, resource_type, resource_id, monthly_eur, status=None, created_at=None):
...
def test_recommendations_summary_totals_survive_past_500_rows(conn):
    ...
    where_clause = (
        "WHERE r.status = 'pending' AND w.resource_type = 'ec2_instance' "
        "AND w.account_id = 'test-coherence'"
    )
...
def _dashboard_kpis(cur):
    ...
    return {
        "pending_count": pending_count,
        "pending_eur": float(potential_monthly),
        "total_waste": float(active_monthly),
        "declined_count": declined_count,
        "declined_monthly": float(declined_monthly),
    }
