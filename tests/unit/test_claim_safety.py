...
def test_guaranteed_savings_not_allowed_for_potential_savings():
    with pytest.raises(FinOpsInvariantError):
        validate_claim_wording(
            "€93,600/year in guaranteed savings", savings_type='potential')
    with pytest.raises(FinOpsInvariantError):
        validate_claim_wording(
            "Save €7,800/month instantly", savings_type='potential')
    with pytest.raises(FinOpsInvariantError):
        validate_claim_wording(
            "Automatic savings with no risk", savings_type='potential')
    # Formulation autorisée
    assert validate_claim_wording(
        "Up to €93,600/year in potential savings identified",
        savings_type='potential') is not None


def test_realized_savings_claim_requires_completed_actions():
    with pytest.raises(FinOpsInvariantError):
        validate_claim_wording(
            "€8,000/month in realized savings",
            savings_type='realized', completed_actions=0)
    assert validate_claim_wording(
        "€8,000/month in realized savings",
        savings_type='realized', completed_actions=3) is not None
