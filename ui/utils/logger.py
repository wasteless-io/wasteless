...
default_logger = setup_logging(log_level=logging.INFO, log_file="logs/wasteless_ui.log")
...
def log_config_change(setting: str, old_value: any, new_value: any, user: str = "streamlit_user"):
    ...
    logger.warning(
        f"Config Change | User: {user} | Setting: {setting} | Old: {old_value} | New: {new_value}"
    )
...
def log_cache_stats(cache_name: str, hits: int, misses: int):
    ...
    logger.debug(
        f"Cache: {cache_name} | Hits: {hits} | Misses: {misses} | Hit Rate: {hit_rate:.1f}%"
    )
