...
RESOURCE_HEADER_RE = re.compile(r'^\s*resource\s+"(?P<type>[\w-]+)"\s+"(?P<name>[\w-]+)"\s*\{')
...
def locate_block(terraform_dir: str, resource_type: str, name: str) -> Optional[tuple]:
    """
    ...
    for tf_file in sorted(directory.glob("*.tf")):
        ...
        for i, line in enumerate(lines):
            ...
            raise TerraformMappingError(f"Unbalanced braces in {tf_file} at line {i + 1}")
