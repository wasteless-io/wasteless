...
def test_short_observation_window_lowers_confidence():
    assert (
        assess_confidence(
            BASE_METADATA, signal_confidence=0.95, observation_days=2, min_observation_days=14
        )
        == "medium"
    )
    assert (
        assess_confidence(
            BASE_METADATA, signal_confidence=0.95, observation_days=14, min_observation_days=14
        )
        == "high"
    )
...
def test_incomplete_metrics_prevent_high_confidence():
    assert (
        assess_confidence(BASE_METADATA, signal_confidence=0.95, metrics_complete=False) == "medium"
    )
    assert assess_confidence(BASE_METADATA, signal_confidence=0.95, metrics_complete=True) == "high"
