...
def test_linear_forecast_matches_elapsed_days():
    ...
    assert linear_forecast(31000, days_elapsed=15, days_in_month=31) == pytest.approx(
        64066.67, abs=0.01
    )
    # Exercice 5 : 15 000 au jour 10 sur 30 jours -> 45 000, pas 30 000
    assert linear_forecast(15000, days_elapsed=10, days_in_month=30) == pytest.approx(45000)
...
def test_forecast_after_remediation_never_below_current_spend():
    with pytest.raises(FinOpsInvariantError):
        forecast_after_remediation(
            forecast=50000, validated_low_risk_savings=40000, current_spend_mtd=45000
        )  # 10 000 < 45 000 déjà dépensés
    # Cas limite : égal au spend déjà réalisé, accepté
    assert (
        forecast_after_remediation(
            forecast=50000, validated_low_risk_savings=5000, current_spend_mtd=45000
        )
        == 45000
    )
