...
def validate_underutilization_action(detector_type: str, resource_type: str, action: str) -> str:
...
def validate_cloudwatch_retention_saving(
    claimed_saving_monthly: float, storage_cost_monthly: float, total_log_cost_monthly: float
) -> float:
...
def validate_eks_resize(
    nodegroup: Optional[str], workload_metrics: Optional[Dict[str, Any]]
) -> bool:
    ...
    if not nodegroup:
        raise FinOpsInvariantError("EKS resize requires a target nodegroup")
    required_metrics = ("cpu", "memory")
    missing = [m for m in required_metrics if not workload_metrics or m not in workload_metrics]
    if missing:
        raise FinOpsInvariantError(f"EKS resize requires workload metrics: missing {missing}")
    return True
