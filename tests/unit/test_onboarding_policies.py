...
CFN_TEMPLATE = os.path.join(ONBOARDING, "cloudformation", "wasteless-onboarding.yaml")
...
class TestPolicyJsons:
    ...
    def test_rollback_snapshot_tagging_is_scoped(self):
        ...
        assert tag["Condition"] == {"StringEquals": {"ec2:CreateAction": "CreateSnapshot"}}
...
class TestCloudFormationDriftGuard:
    ...
    def _inline_policies(role):
        return {p["PolicyName"]: p["PolicyDocument"] for p in role["Properties"]["Policies"]}
    ...
    def test_remediation_role_matches_both_jsons(self):
        ...
        assert inline["wasteless-readonly"]["Statement"] == load_json(READONLY_JSON)["Statement"]
        assert (
            inline["wasteless-remediation"]["Statement"] == load_json(REMEDIATION_JSON)["Statement"]
        )
