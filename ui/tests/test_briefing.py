...
SAMPLE_DATA = {
    "date": "2026-07-05",
    "active_waste": {"monthly_eur": 10.53, "count": 11, "by_type": [("ebs_snapshot", 9, 9.57)]},
...
class TestBriefingCache(unittest.TestCase):
    ...
    def test_cache_hit_with_dict_rows(self):
        created = datetime(2026, 7, 5, 8, 0)
        conn = FakeConn(rows=[{"content": "cached text", "model": "m", "created_at": created}])
        result = daily_briefing.get_or_create_briefing(conn)
        self.assertEqual(result["content"], "cached text")
    ...
    def test_refresh_skips_cache_and_regenerates(self):
        ...
        with (
            patch.object(daily_briefing.llm, "is_enabled", return_value=True),
            patch.object(daily_briefing, "collect_briefing_data", return_value=SAMPLE_DATA),
            patch.object(daily_briefing, "generate_briefing", return_value="fresh text") as gen,
        ):
