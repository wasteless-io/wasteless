...
def _fake_config(enabled=True, threshold=50.0, required_types=None):
    ...
    config.requires_pr = MagicMock(side_effect=lambda rtype, savings: (
...
class TestMaybeOpenPR(unittest.TestCase):
    ...
    def test_below_threshold_not_routed(self):
        with patch.object(tf_pr, '_load_backend_config',
                          return_value=_fake_config(threshold=50.0)):
            result = tf_pr.maybe_open_pr(self.conn, 1, _row(savings=3.65),
                                         dry_run=True)
    ...
    def test_unmanaged_resource_falls_back(self):
        ...
        with patch.object(tf_pr, '_load_backend_config',
             ...
             patch('src.remediators.terraform_pr.TerraformPRRemediator',
                   return_value=remediator):
            result = tf_pr.maybe_open_pr(self.conn, 1, _row(), dry_run=True)
        self.assertIsNone(result)

    def test_routed_real_run_stores_pr_url(self):
        proposal = MagicMock(pr_url='https://github.com/acme/infra/pull/7',
    ...
    def test_unsafe_change_surfaces_error_without_fallback(self):
        ...
        remediator = MagicMock()
        remediator.propose_removal.side_effect = TerraformPRError(
            'dangling references')
        with patch.object(tf_pr, '_load_backend_config',
                          return_value=_fake_config()), \
             patch('src.remediators.terraform_pr.TerraformPRRemediator',
...
class TestSyncOpenPRs(unittest.TestCase):
    ...
    def test_merged_pr_approves(self):
        conn, cursor = self._conn_with_prs(
            [{'id': 1, 'pr_url': 'https://github.com/a/b/pull/1'}])
        with patch.object(tf_pr.subprocess, 'run',
                          return_value=self._gh('MERGED')):
...
class TestTerraformPRConfigFields(unittest.TestCase):
    ...
    def test_set_and_persist_repo(self):
        self.assertTrue(self.manager.set_terraform_pr_field(
            'repo', 'acme/infra'))
