...
class SavingsTracker:
    ...
    def get_instance_cost_for_period(
        ...
        try:
            logger.debug(
                f"Getting cost for {instance_id} " f"from {start_date.date()} to {end_date.date()}"
            )
    ...
    def calculate_average_daily_cost(self, instance_id: str, days_back: int = 30) -> float:
        ...
        logger.info(
            f"Instance {instance_id}: " f"${avg_daily_cost:.2f}/day average ({days_back} days)"
        )
    ...
    def verify_savings_for_action(self, action_log_id: int) -> Optional[Dict]:
        ...
        if days_since_action < 7:
            logger.info(
                f"⏳ Only {days_since_action} days since action, " f"need 7+ for verification"
            )
            cursor.close()
            return None

        logger.info(
            f"Analyzing savings for {instance_id} " f"({days_since_action} days after stop)"
        )
    ...
    def verify_all_unverified_actions(self, min_days_elapsed: int = 7) -> List[Dict]:
        ...
        """
        logger.info(f"🔍 Finding unverified actions " f"(older than {min_days_elapsed} days)")
