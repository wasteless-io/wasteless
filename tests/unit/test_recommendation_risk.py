...
def test_production_resource_requires_higher_risk_than_dev():
    assert minimum_risk_for("resize", "production", owner="team-x") != minimum_risk_for(
        "resize", "dev", owner="team-x"
    )
...
def test_unknown_environment_destructive_action_is_high_risk():
    ...
    floor = minimum_risk_for(
        recommendation["recommended_action"], recommendation["environment"], recommendation["owner"]
    )
    levels = ("low", "medium", "high", "critical")
...
def test_production_idle_detection_requires_minimum_observation_window():
    ...
    assert validate_observation_window("production", observation_days=14, action="stop") == 14
...
def test_dev_idle_detection_accepts_shorter_observation_window():
    ...
    assert validate_observation_window("dev", observation_days=7, action="stop") == 7
    with pytest.raises(FinOpsInvariantError):
...
def test_batch_workload_not_marked_idle_without_schedule_context():
    ...
    assert (
        validate_batch_workload_classification(
            is_batch_workload=True, has_schedule_context=True, action="stop"
        )
        is True
    )
    assert (
        validate_batch_workload_classification(
            is_batch_workload=False, has_schedule_context=False, action="stop"
        )
        is True
    )
...
def test_rds_delete_never_recommended_by_underutilization_detector():
    ...
    assert validate_underutilization_action("rds_underutilized", "rds", "downsize") == "downsize"
...
def test_cloudwatch_retention_change_is_not_counted_as_full_log_cost_saving():
    ...
    with pytest.raises(FinOpsInvariantError):
        validate_cloudwatch_retention_saving(
            claimed_saving_monthly=2000, storage_cost_monthly=800, total_log_cost_monthly=2000
        )
    # Ni égaler/dépasser le coût total des logs
    with pytest.raises(FinOpsInvariantError):
        validate_cloudwatch_retention_saving(
            claimed_saving_monthly=2000, storage_cost_monthly=2000, total_log_cost_monthly=2000
        )
    assert (
        validate_cloudwatch_retention_saving(
            claimed_saving_monthly=500, storage_cost_monthly=800, total_log_cost_monthly=2000
        )
        == 500
    )
...
def test_eks_resize_requires_nodegroup_and_workload_context():
    ...
    assert (
        validate_eks_resize(nodegroup="ng-1", workload_metrics={"cpu": 0.3, "memory": 0.4}) is True
    )
