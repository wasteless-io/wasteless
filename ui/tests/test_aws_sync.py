...
def make_client_factory(
    instances=None, volumes=None, addresses=None, snapshots=None,
    nat_gateways=None, elbv2_arns=None, classic_names=None,
    fail_regions=None,
):
    ...
    def factory(service, region_name=None):
        ...
        if service == 'ec2':
            ...
            client.describe_nat_gateways.return_value = {
                'NatGateways': nat_gateways or []
            }
        elif service == 'elbv2':
            client.get_paginator.return_value = _paginator(
                [{'LoadBalancers': [{'LoadBalancerArn': a}
                                    for a in (elbv2_arns or [])]}]
            )
        elif service == 'elb':
            client.get_paginator.return_value = _paginator(
                [{'LoadBalancerDescriptions': [{'LoadBalancerName': n}
                                               for n in (classic_names or [])]}]
            )
        return client
...
class TestFindVanishedResources(unittest.TestCase):
    ...
    def test_existing_resources_not_vanished(self):
        ...
        vanished = find_vanished_resources(
            ...
            regions=REGIONS, client_factory=factory,
        )
    ...
    def test_terminated_instance_vanished(self):
        factory = make_client_factory(instances=[
    ...
    def test_deleted_nat_gateway_vanished(self):
        factory = make_client_factory(nat_gateways=[
    ...
    def test_empty_pending(self):
        vanished = find_vanished_resources(
            {}, regions=REGIONS, client_factory=make_client_factory(),
        )
        self.assertEqual(vanished, {})
